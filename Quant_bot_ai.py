...
class AIAnalyst:
    """
    Uses Claude to analyze market data and generate trading decisions.
...
{{
  ...
  "risk_warning": "Most important risk note or null"
        ...
        try:
            ...
            log.error(f"Claude returned invalid JSON for {symbol}: {e}\nRaw: {text}")
    ...
    def _fallback(reason: str) -> dict:
        return {
            "action": "HOLD",
            "confidence": 0,
            "reasoning": f"AI unavailable — defaulting to HOLD. Reason: {reason}",
            "key_signals": [],
...
class QuantTradingBot:
    ...
    def run_continuous(self):
        log.info(
            f"Bot started | Capital: ${self.config.initial_capital:,.2f} | "
